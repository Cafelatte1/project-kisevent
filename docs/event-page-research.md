...
| 병렬 | Claude Code에서는 `.claude/agents/event-analyzer.md`(Sonnet, 도구 2개)에 image_id를 2개씩 묶어 넘기고 묶음마다 하나씩 띄운다(고정비 절감; 2026-09-18 변경). 서브에이전트가 없는 Claude Desktop은 같은 두 호출을 직접 순서대로 |
