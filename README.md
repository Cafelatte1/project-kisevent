...
- **서브에이전트 `event-analyzer`** — 배너 1장을 읽어 대상·기준 JSON을 만들고 저장한다. Claude Code에서는 `.claude/agents/event-analyzer.md`가 미요약 이미지 5장씩을 맡아 병렬로 뜬다. 서브에이전트가 없는 환경(Claude Desktop)에서는 메인 세션이 같은 일을 직접 한다.
