...
2. Claude Code 또는 Desktop에서: "현재 뱅키스 고객대상 이벤트 뭐가 있어?" — `sync_now`로 수집한 뒤 미요약 배너를 먼저 요약(Claude Code는 `event-analyzer` 서브에이전트를 이미지 5장씩 병렬로 띄움)한 뒤 목록을 답한다. 진행중 30여 건 기준 몇 분 걸린다.
