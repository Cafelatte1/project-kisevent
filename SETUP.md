...
**Windows 지름길**: `install_ps.bat` 하나만 받아 아무 위치에서 더블클릭하면 git 설치·클론(`%USERPROFILE%\project-kisevent`)부터 1·2·3·5단계(uv 설치, `uv sync`, 작업 스케줄러 등록·서버 기동, Claude Desktop 설정 병합)까지 한 번에 한다. 이미 클론한 폴더 안에서 실행하면 그 폴더를 쓴다. 이후 코드 업데이트는 `update_ps.bat` 더블클릭 — 최신 코드 받기, `uv sync`, 서버 재시작까지 한다(Claude Desktop은 재시작해야 새 도구 설명을 읽는다). 끝나면 Claude Desktop을 완전히 재시작하고 6단계로 간다. 아래는 같은 일을 손으로 할 때의 절차다.

**사내 PC라 PowerShell이나 winget이 막혔다면** `install_cmd.bat`을 쓴다. PowerShell도 git도 쓰지 않고 `curl.exe`·`tar.exe`(Windows 10 1803 이상 기본 포함)로 소스 zip과 uv를 받아 같은 결과를 만든다. 업데이트는 `update_cmd.bat`.
...
## 3. 서버 자동 실행 등록
서버 하나가 대시보드 + MCP(HTTP) + 수집기를 같이 띄운다. 수집은 자동으로 돌지 않고 에이전트가 질문에 답하기 전에 `sync_now`로 요청할 때만 돈다. 로그온 시 창 없이 시작되도록 작업 스케줄러에 등록한다.
```powershell
...
```
`{"ok":true,...}`가 나와야 한다. 기동 직후에는 데이터가 비어 있고, 첫 질문 때 에이전트가 수집한다. 지난 이벤트(최근 100건, 이미지 약 250MB)는 필요할 때만 대시보드의 "지난 이벤트 100건 수집" 버튼으로 받는다 — 없어도 현재 이벤트 조회에는 지장이 없다.
